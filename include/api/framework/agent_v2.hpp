...
inline Agent::Value Agent::get(Unit unit) {
    if (_data_producer && _component_data) {
        return _data_producer(unit, _component_data.get());
    }
    // Return empty value if no producer function
    return Value();
}
...
inline void Agent::handle_response(Message* msg) {
    if (_response_handler && _component_data) {
        _response_handler(msg, _component_data.get());
    }
    // Do nothing if no handler function (default behavior)
}
